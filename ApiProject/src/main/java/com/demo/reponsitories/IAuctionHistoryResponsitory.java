...
public interface IAuctionHistoryResponsitory extends CrudRepository<Auctionhistory, Integer> {
	...
	@Query("select new com.demo.Dtos.Output.AuctionOutput(auction.id, auction.eventdate) from Auctionhistory where account.id = id")
	public List<AuctionOutput> getAuctionByIdCustomer(@Param("id") int idCus);
