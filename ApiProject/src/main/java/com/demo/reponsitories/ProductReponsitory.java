...
public interface ProductReponsitory extends CrudRepository<Product, Integer> {
	...
	@Query(value = "select new com.demo.Dtos.Output.ProductOutput(id, category.name, name ,priceMin,  image, description, status, isAccept) from Product where name like %:name% and isDelete = 0 "
